...
export function renderList(residentes){
    ...
    const tbody = table.querySelector('tbody');
        ...
        residentes.forEach(residente => {
            ...
            tr.innerHTML = `
                <td>${residente.cedula_residente && residente.cedula_residente.trim() !== '' ? residente.cedula_residente : 'No aplica'}</td>
                <td>${residente.nombre_residente}</td>
                <td>${residente.apellido_residente}</td>
                <td>${residente.telefono}</td>
                <td>${residente.direccion}</td>
                <td>
                    <button class="btn btn-info btn-secondary btn-sm" data-id="${residente.id_residente}">Ver Info.</button>
